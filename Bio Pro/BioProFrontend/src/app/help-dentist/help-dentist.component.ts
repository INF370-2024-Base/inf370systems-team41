...
export class HelpDentistComponent  implements OnInit {
  ...
  filterContent(): void {
    ...
    contentElements.forEach((element: HTMLElement) => {
      const containerType = element.classList.contains('search-dentits') ? 'search-dentists' :
                            element.classList.contains('add-dentits') ? 'add-dentits' :
                            element.classList.contains('edit-dentits') ? 'edit-dentits' : 
                            element.classList.contains('delete-dentits') ? 'delete-dentits' : '';
